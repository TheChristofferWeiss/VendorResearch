...
class SimpleScraper:
    def __init__(self):
        ...
        self.max_pages = 50  # Limit to prevent infinite crawling
    
    def scrape_entire_site(self, base_url):
        """Scrape entire website, excluding blogs"""
        try:
            ...
            # Filter out blog URLs, legal pages, and non-English pages
            filtered_urls = [url for url in all_urls if not self._is_blog_url(url) and not self._is_legal_page(url) and self._is_english_page(url)]
            
            print(f"Found {len(filtered_urls)} non-blog pages to scrape")
            
            # Scrape each URL
            all_content = []
            for i, url in enumerate(filtered_urls[:self.max_pages]):
                print(f"Scraping page {i+1}/{min(len(filtered_urls), self.max_pages)}: {url}")
    ...
    def _discover_site_urls(self, base_url):
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                # Clean URL (remove fragments and query parameters for now)
    ...
    def _is_legal_page(self, url):
        """Check if URL is a legal/administrative page"""
    ...
    def _is_english_page(self, url):
        """Check if URL is likely an English language page"""
    ...
    def scrape_url(self, url):
        try:
            ...
            if result.returncode == 0:
                ...
                if any(blocked in content_lower for blocked in ["access denied", "permission", "blocked", "suspicious", "firewall", "your request was blocked"]):
                ...
                if clean_content:
                    ...
                    # Check raw HTML for blocking/error indicators
                    if "# Index" in html_content or "blocked" in html_content.lower():
                        print(f"Skipping {url}: blocked or placeholder content")
                        return None
                    return html_content
            else:
    ...
    def _extract_with_trafilatura(self, html_content):
        try:
            ...
            print(f"Error with trafilatura extraction: {e}")
            return None
...
class SimpleWebHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _render_vendor_table(self, vendors):
        if not vendors:
            return "<tr><td colspan='6'>No vendors found.</td></tr>"
        
        html = ""
        for vendor in vendors:
            html += f"""
            <tr>
                <td><input type="checkbox" class="vendor-checkbox" value="{vendor[0]}"></td>
                <td>{vendor[0]}</td>
                <td>{vendor[1]}</td>
    ...
    def _start_scraping(self, vendor_id, url):
        try:
            ...
            # Scrape entire site (excluding blogs)
            all_content = self.scraper.scrape_entire_site(url)
            
            if all_content:
                # Save all pages to files
                self._save_all_pages_to_files(vendor_id, all_content)
